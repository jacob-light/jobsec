...
#' @param by a string specifying what locality type to organize by: "city" or "county"
...
#' @param cities a character vector of counties to filter for.
...
#' @examples
#' df <- warnExtract(by = "county", counties = c("Santa Clara", 
...
warnExtract <- function(start_date = -Inf , 
  ...
  if(!base::is.na(cities[1])){
    if(!base::all(base::is.element(cities, base::unique(df_ca_warn_mod$city)))){
      stop("City not found")
    }
  }
  ...
  df_ca_warn_mod <-
    ...
    dplyr::select(type_date, by, layoff_reason, n_employees) %>%
    dplyr::group_by(!!as.name(type_date), !!as.name(by), layoff_reason) %>%
